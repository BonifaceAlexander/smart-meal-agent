# Placeholder vector index wrapper. Replace with Chroma/pgvector/FAISS
# integration.
class VectorMenuIndex:
    ...
    def search(self, pref, top_k=50):
        ...
        for it in self.items:
            if pref is not None:
                if (
                    pref.budget_cents is not None
                    and it.get("price_cents", 0) > pref.budget_cents
                ):
                    continue
                if (
                    pref.max_calories is not None
                    and it.get("calories", 0) > pref.max_calories
                ):
                    continue
                if pref.dietary_restrictions:
                    ...
                    for d in pref.dietary_restrictions:
                        if (
                            d.lower() not in [t.lower() for t in it.get("tags", [])]
                            and d.lower() != "vegetarian"
                        ):
